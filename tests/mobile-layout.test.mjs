...
test('responsive light atlas keeps every major surface inside the viewport', async () => {
  ...
  assert.match(polishCss, /@media\s*\(max-width:\s*760px\)[\s\S]*?\.page-head h1\s*\{[\s\S]*?font-size:\s*clamp\(1\.9rem,\s*8\.8vw,\s*2\.6rem\);[\s\S]*?word-break:\s*normal;[\s\S]*?overflow-wrap:\s*anywhere;[\s\S]*?text-wrap:\s*pretty/);
