...
test('responsive light atlas keeps every major surface inside the viewport', async () => {
  ...
  assert.match(polishCss, /@media\s*\(max-width:\s*760px\)[\s\S]*?\.page-head h1\s*\{[\s\S]*?font-size:\s*clamp\(2\.1rem,\s*9\.5vw,\s*2\.75rem\);[\s\S]*?word-break:\s*keep-all;[\s\S]*?text-wrap:\s*balance/);
